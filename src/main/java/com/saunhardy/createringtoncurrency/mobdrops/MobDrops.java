...
public class MobDrops {
    ...
    public static void onMobDeath(LivingDeathEvent event) {
        ...
        int enchantmentLevel = stack.isEmpty() ? 0 :
                stack.getEnchantmentLevel(ModEnchantments.CAPITALIST_GREED.get());
        ...
        if (backendLimitReached.contains(uuid)) {
            if (!warnedToday.contains(uuid)) {
                player.sendSystemMessage(limitMessage());
    ...
    private static void checkBackendLimitOnce(UUID uuid, ServerPlayer player) {
        ...
        EXECUTOR.submit(() -> {
            try {
                ...
                String apiUrl = MoneyCommands.safeJoin(Config.API_BASE_URL.get(),
                        Config.API_MOB_LIMIT_URL.get() + "?uuid=" + uuid);
    ...
    private static void sendLimitReached(ServerPlayer player) {
        ...
        EXECUTOR.submit(() -> {
            try {
                ...
                // Consume response without an empty-body while loop
    ...
    private static Component limitMessage() {
        return Component.literal("⚠ You've reached today's mob farming limit ($1000).")
