...
public class DepositorMenu extends AbstractContainerMenu {
    ...
    public DepositorMenu(int id, Inventory inv, DepositorTerminalBlockEntity be, boolean ownerMode) {
        this(id, inv, be.getBlockPos(), ownerMode, be.getPriceDenomination(), be.getPriceCount(), be.getOwnerName(), be.getStorage());
    }
    ...
    public static DepositorMenu fromBuf(int id, Inventory inv, RegistryFriendlyByteBuf buf) {
        ...
        return new DepositorMenu(id, inv, pos, ownerMode, priceDenomination, priceCount, ownerName, storage);
    }

    private DepositorMenu(int id, Inventory inv, BlockPos pos, boolean ownerMode, int priceDenomination, int priceCount,
    ...
    public boolean isOwnerMode() { return ownerMode; }
    ...
    public boolean stillValid(@NotNull Player player) {
        return player.distanceToSqr(pos.getCenter()) <= DepositorTerminalBlock.MAX_USE_DISTANCE_SQ;
    }

    @Override
    public @NotNull ItemStack quickMoveStack(@NotNull Player player, int index) {
        if (!ownerMode) return ItemStack.EMPTY;
